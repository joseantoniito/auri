...
class Roles_model extends CRM_Model
{
    ...
    public function update($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if(total_rows('tblrolepermissions',array('roleid'=>$id)) == 0){
            ...
            } else if(total_rows('tblrolepermissions',array('roleid'=>$id)) != count($all_permissions)){
                 foreach($all_permissions as $p ){
                    if(total_rows('tblrolepermissions',array('roleid'=>$id,'permissionid'=>$p['permissionid'])) == 0){
                        $_ins = array();
                        $_ins['roleid'] = $id;
                        $_ins['permissionid'] = $p['permissionid'];
                        $this->db->insert('tblrolepermissions',$_ins);
            ...
            foreach($all_permissions as $permission){
                foreach($this->perm_statements as $c){
                    ...
                    $this->db->update('tblrolepermissions',array('can_'.$c=>0));
                    if($this->db->affected_rows() > 0){
            ...
            foreach($permissions as $key=>$val){
                foreach($val as $p){
                    ...
                    $this->db->update('tblrolepermissions',array('can_'.$key=>1));
                    if($this->db->affected_rows() > 0){
