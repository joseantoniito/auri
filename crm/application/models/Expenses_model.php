...
class Expenses_model extends CRM_Model
{
    ...
    {
        $this->db->select('*,tblexpenses.id as id,tblexpensescategories.name as category_name,tblinvoicepaymentsmodes.name as payment_mode_name,tbltaxes.name as tax_name, tblexpenses.id as expenseid');
    ...
    public function get_expenses_total($data)
    {
        ...
        if (isset($data['currency'])) {
            ...
            if(total_rows('tblexpenses',array('currency'=>$base_currency,'clientid'=>$data['customer_id']))){
                $currency_switcher = true;
            }
        }
            ...
            $currencyid = $base_currency;
            if(total_rows('tblexpenses',array('currency !='=>$base_currency))){
                $currency_switcher = true;
            }
        }
    ...
    public function convert_to_invoice($id,$draft_invoice = false,$params = array())
    {
        ...
        $new_invoice_data['billed_expenses'][1] = array($expense->expenseid);
