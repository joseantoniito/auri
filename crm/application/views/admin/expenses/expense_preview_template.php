...
<div class="tab-content">
    <div role="tabpanel" class="tab-pane ptop10 active" id="tab_expense">
        <div class="row">
            ...
            if ($recurring_expense->custom_recurring == 0) {
                $recurring_expense->recurring_type = 'MONTH';
            }
            ...
            ?>
            <div class="col-md-12">
              <h4 class="font-medium text-success"><?php echo _l('next_expense_date',$next_date); ?>
                <?php
                if($expense->recurring_from != NULL){ ?>
                <?php echo '<br /><small class="text-muted">'._l('expense_recurring_from','<a href="#" onclick="init_expense('.$expense->recurring_from.');return false;">'.$recurring_expense->category_name.(!empty($recurring_expense->expense_name) ? ' ('.$recurring_expense->expense_name.')' : '').'</small></a>'); ?>
                <?php } ?>
            </h4>
        </div>
    </div>
...
<div role="tabpanel" class="tab-pane" id="tab_child_expenses">
 ...
 <ul class="list-group">
   ...
   <li class="list-group-item">
     <a href="#" onclick="init_expense(<?php echo $recurring->expenseid; ?>); return false;" target="_blank"><?php echo $recurring->category_name.(!empty($recurring->expense_name) ? ' ('.$recurring->expense_name.')' : ''); ?>
     </a>
     <br />
     <span class="inline-block mtop10">
      <?php echo '<span class="bold">'._d($recurring->date).'</span>'; ?><br />
      <p><span class="bold"><?php echo _l('expense_amount'); ?></span> <span class="text-danger bold font-medium"><?php echo format_money($recurring->amount,$recurring->currency_data->symbol); ?></span>
       <?php if($recurring->tax != 0){
        echo '<br /><span class="bold">'._l('expense_tax') .'</span> ' . $recurring->taxrate . ' ('.$recurring->tax_name.')';
        $total = $recurring->amount;
        $_total = ($total / 100 * $recurring->taxrate);
        $total += $_total;
        echo ' - <b class="text-danger">' . format_money($total,$recurring->currency_data->symbol) . '</b>';
    }
    ?>
</span>
</li>
...
<?php } else { ?>
<p><?php echo _l('no_child_found',_l('expenses')); ?></p>
<?php } ?>
</div>
<?php } ?>
...
<div role="tabpanel" class="tab-pane" id="tab_reminders">
    <a href="#" data-toggle="modal" data-target=".reminder-modal-expense-<?php echo $expense->id; ?>"><i class="fa fa-bell-o"></i> <?php echo _l('expense_set_reminder_title'); ?></a>
    <hr />
...
<script>
    ...
    initDataTable('.table-reminders', admin_url + 'misc/get_reminders/' + <?php echo $expense->id ;?> + '/' + 'expense', [4], [4]);
