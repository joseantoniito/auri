       ...
       if($expense->recurring > 0 || $expense->recurring_from != NULL) {
        ...
        <div class="col-md-12">
          <h4 class="font-medium text-success no-mtop">
           ...
           <?php echo _l('next_expense_date',_d($next_date)); ?><br />
           <?php } ?>
           <?php
           if($expense->recurring_from != NULL){ ?>
           <?php echo '<small class="text-muted">'._l('expense_recurring_from','<a href="#" onclick="init_expense('.$expense->recurring_from.');return false;">'.$recurring_expense->category_name.(!empty($recurring_expense->expense_name) ? ' ('.$recurring_expense->expense_name.')' : '').'</small></a>'); ?>
           <?php } ?>
         </h4>
       </div>
     </div>
