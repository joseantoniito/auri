...
<div class="modal-body">
   <div class="row">
      <div class="col-md-8 task-single-col-left">
         ...
         <div class="clearfix"></div>
         <hr />
         <a href="#" onclick="add_task_checklist_item(); return false" class="mbot10 inline-block">
            ...
            <div class="col-md-12" id="attachments">
               ...
               <ul class="list-unstyled">
                  ...
                  <li class="mbot10 task-attachment">
                     ...
                     <a href="<?php echo $href_url; ?>" class="pull-left" target="_blank">
                     <?php if($is_image){ ?>
            ...
            <div id="task-comments">
               <?php
                  ...
                  foreach ($task->comments as $comment) {
                    $comments .= '<div data-commentid="' . $comment['id'] . '" class="tc-content">';
                    $comments .= '<span class="mtop5 text-muted">' . time_ago($comment['dateadded']) . '</span>';
                  ...
                  $comments .= '<div class="comment-content mtop15">'.check_for_links($comment['content']) . '</div>';
                  $comments .= '</div>';
                  if ($i >= 0 && $i != $len - 1) {
      ...
      <div class="col-md-4 task-single-col-right">
         <div class="pull-right mbot10 task-single-menu task-menu-options">
            <div class="content-menu hide">
               <ul>
                  <?php if(has_permission('tasks','','edit')) { ?>
                  <li>
                     <a href="#" onclick="edit_task(<?php echo $id; ?>); return false;">
         ...
         <?php if(has_permission('tasks','','edit') || has_permission('tasks','','create')){ ?>
         <select data-width="100%" class="text-muted selectpicker task-action-select mbot10" name="select-followers" data-live-search="true" title='<?php echo _l('task_single_followers_select_title'); ?>' data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
         <?php
            $options = '';
...
<script>
   ...
   document.getElementById("dropbox-chooser-task").appendChild(Dropbox.createChooseButton({
           success: function(files) {
              $.post(admin_url+'tasks/add_external_attachment',{files:files,task_id:taskid,external:'dropbox'}).done(function(){
