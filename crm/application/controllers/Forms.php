        ...
        if ($this->input->post('key')) {
            if ($this->input->post('key') == $key) {
                ...
                if ($insert_to_db == TRUE) {
                    ...
                    do_action('lead_created',array('lead_id'=>$lead_id,'web_to_lead_form'=>true));
                    ...
                    if ($lead_id) {
                        ...
                        if (!empty($form->notify_type) || $form->notify_type == 'assigned') {
                            $ids = unserialize($form->notify_ids);

                            if ((!is_array($ids) || count($ids) == 0) && $form->notify_type != 'assigned') {
                                return;
                            }
