...
class Estimates extends Admin_controller
{
    ...
    public function estimate($id = '')
    {
        ...
        if($id != ''){
            if(total_rows('tblclients',array('active'=>0,'userid'=>$data['estimate']->clientid)) > 0
     ...
     public function update_number_settings($id){
        ...
        if($this->input->post('prefix')){
            $this->db->where('id',$id);
            $this->db->update('tblestimates',array('prefix'=>$this->input->post('prefix')));
     ...
     public function pipeline_load_more()
    {
        ...
        $estimates = $this->estimates_model->do_kanban_query($status,$this->input->get('search'),$page,array('sort_by'=>$this->input->get('sort_by'),'sort'=>$this->input->get('sort')));
