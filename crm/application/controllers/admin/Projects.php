...
class Projects extends Admin_controller
{
    ...
    public function project($id = '')
    {
        ...
        if($id != ''){
            if(total_rows('tblclients',array('active'=>0,'userid'=>$data['project']->clientid)) > 0
                || (total_rows('tblcustomeradmins',array('staff_id'=>get_staff_user_id(),'customer_id'=>$data['project']->clientid)) == 0 && !has_permission('customers','','view'))){
    ...
    public function export_project_data($id){
        if(has_permission('projects','','create')){
           ...
           $data['total_files_attached'] = total_rows('tblprojectfiles',array('project_id'=>$project->id));
           $data['total_discussion'] = total_rows('tblprojectdiscussions',array('project_id'=>$project->id));
