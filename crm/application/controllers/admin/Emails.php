...
class Emails extends Admin_controller
{
    ...
    public function index()
    {
        ...
        foreach($this->perfex_base->get_available_languages() as $av_language){
            if($av_language != 'english'){
                foreach($email_templates_english as $template){
                    if(total_rows('tblemailtemplates',array('slug'=>$template['slug'],'language'=>$av_language)) == 0){
                        $data = array();
        ...
        $data['staff']     = $this->emails_model->get(array('type'=>'staff','language'=>'english'));
        $data['tasks']     = $this->emails_model->get(array('type'=>'tasks','language'=>'english'));
        $data['client']    = $this->emails_model->get(array('type'=>'client','language'=>'english'));
        $data['tickets']   = $this->emails_model->get(array('type'=>'ticket','language'=>'english'));
        $data['invoice']   = $this->emails_model->get(array('type'=>'invoice','language'=>'english'));
        $data['estimate']  = $this->emails_model->get(array('type'=>'estimate','language'=>'english'));
