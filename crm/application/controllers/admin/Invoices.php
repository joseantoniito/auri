...
class Invoices extends Admin_controller
{
    ...
    public function update_number_settings($id){
        ...
        if($this->input->post('prefix')){
            ...
            $this->db->where('id',$id);
            $this->db->update('tblinvoices',array('prefix'=>$this->input->post('prefix')));
    ...
    public function validate_invoice_number(){
       ...
       if(total_rows('tblinvoices',array('YEAR(date)'=>date('Y',strtotime(to_sql_date($date))),'number'=>$number)) > 0){
            echo 'false';
       } else {
            echo 'true';
       }
    ...
    public function invoice($id = '')
    {
        ...
        if($id != ''){
            if(total_rows('tblclients',array('active'=>0,'userid'=>$data['invoice']->clientid)) > 0
                || (total_rows('tblcustomeradmins',array('staff_id'=>get_staff_user_id(),'customer_id'=>$data['invoice']->clientid)) == 0 && !has_permission('customers','','view'))){
    ...
    public function get_invoices_total()
    {
        if ($this->input->post()) {
            ...
            if(!$this->input->post('customer_id')){
                ...
                $_data['customer_id'] = $this->input->post('customer_id');
                $multiple_currencies = call_user_func('is_client_using_multiple_currencies',$this->input->post('customer_id'));
            }
            ...
            $data['total_result'] = $this->invoices_model->get_invoices_total($_data);
