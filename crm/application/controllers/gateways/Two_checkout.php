     ...
     if ($this->input->post()) {
        ...
        if ($oResponse->isSuccessful()) {
            ...
            if ($oResponse['response']['responseCode'] == 'APPROVED') {
                ...
                $payment_data['paymentmode']   = $this->two_checkout_gateway->get_id();
   ...
   <body>
      ...
      <script>
         ...
         var successCallback = function(data) {
             ...
             myForm.submit();
            // $('#2checkout_form').find('button[type="submit"]').addClass('disabled');
         };
         ...
         var args = {
         sellerId: "<?php echo get_option('paymentmethod_two_checkout_account_number'); ?>",
         publishableKey: "<?php echo get_option('paymentmethod_two_checkout_publishable_key'); ?>",
         ccNo: $("#ccNo").val(),
         cvv: $("#cvv").val(),
         ...
         TCO.loadPubKey('<?php if(get_option('paymentmethod_two_checkout_test_mode_enabled') == 1){echo 'sandbox';}else{echo 'production';} ?>');
